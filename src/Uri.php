...
class Uri {
     ...
     */
    public function __construct($uri = '', $prependScheme = false)
    {
        if ($uri != null) {
            $parts = parse_url($uri);
            if ($prependScheme && empty($parts['scheme'])) {
                $uri = 'http://' . $uri;
                $parts = parse_url($uri);
            }
            if ($parts === false) {
     ...
     * @return UriInterface
     */
    public static function resolve(Uri $base, $rel)
    {
        if ($rel === null || $rel === '') {
            return $base;
        }
     ...
     * @return UriInterface
     */
    public static function withoutQueryValue(Uri $uri, $key)
    {
     ...
     * @return UriInterface
     */
    public static function withQueryValue(Uri $uri, $key, $value)
    {
        $current = $uri->getQuery();
    ...
    {
        return $this->path == null ? '' : pathinfo($this->path, PATHINFO_BASENAME);
    }

    public function getExtension()
    {
        return $this->path == null ? '' : pathinfo($this->path, PATHINFO_EXTENSION);
    }
     ...
     * @return string
     */
    private static function createUriString($scheme, $authority, $path, $query, $fragment = null)
    {
        $uri = '';
